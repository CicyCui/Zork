		...
		try {
			...
			while(true){
				...
				for(Trigger myTrigger: triggers.keySet()){	
					 if(command.checkTriggers(input,myTrigger,ZorkMap,inventory)){
				...
				for(Trigger tri: curr.triggers){
					if(command.checkTriggers(input,tri,ZorkMap,inventory)){
						roomTrigger = true;
					}
				}
				if(!roomTrigger){
				...
				if(input.equals("n")){
					...
					if(input.length() > 7){
						...
						if(inventory.contains(obj))
						{
							...
							if(cmd == null){
								...
								command.executeCmd(cmd, ZorkMap);
								...
								for(Trigger mytri: triggers.keySet()){
									...
									command.checkTriggers(curritem.getName(),mytri,ZorkMap,inventory);
								}
								//}
					...
					if(input.split(" ").length >= 4){
						...
						if(ZorkMap.keySet().contains("creature" + obj) && curr.creatures.contains(obj)){
							...
							if(creature.getVulnerability().equals(item)){
				...
				for(Trigger myTrigger: triggers.keySet()){
					command.checkTriggers(input,myTrigger,ZorkMap,inventory);
				}
				}
				}
			 }
